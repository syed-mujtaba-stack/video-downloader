      ...
      <div className="flex items-center justify-between">
        <label className="text-xs font-bold text-white flex items-center gap-1.5 cursor-pointer">
          <input
            ...
            className="w-4 h-4 rounded text-indigo-600 bg-slate-800 border-white/20 focus:ring-indigo-500"
          />
      ...
      {isEnabled && (
        <div className="space-y-4 pt-1 animate-in fade-in duration-200">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-[11px] text-slate-400">
                <span>Start:</span>
                <span className="font-mono text-white font-semibold">{formatTime(startTime)}</span>
              </div>
              <input
                type="range"
                min={0}
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-[11px] text-slate-400">
                <span>End:</span>
                <span className="font-mono text-white font-semibold">{formatTime(endTime)}</span>
              </div>
              <input
                type="range"
                min={startTime + 1}
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-[10px] uppercase font-semibold text-slate-500">Quick:</span>
            {duration >= 30 && (
              <button
                type="button"
                onClick={() => {
